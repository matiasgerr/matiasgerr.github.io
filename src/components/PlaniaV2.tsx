import planiaImg from "../assets/6xQHC.jpg";

interface YipiDrawerProps {
  open: boolean;
}

const PlaniaV2 = ({ open }: YipiDrawerProps) => (
  <div
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8 md:gap-16">
          <div className="md:w-1/2 flex flex-col justify-center">
            ...
            <h3 className="text-2xl md:text-3xl font-bold text-slate-100 mb-4">
              Plania V2 Aplication
            </h3>
            <p className="text-slate-400 text-sm md:text-base leading-relaxed mb-6">
              Aplicación basada en React Native que tiene como objetivo hacer
              una gestion total de locales a tu cargo mas consumo, costos,
              productos, servicios aparte de una integracion de plan de pago,
              etc.
            </p>
            <div className="flex flex-wrap gap-2 font-mono text-xs">
              {["React Native", "Tailwind", "REST"].map((tech) => (
                <span
                  key={tech}
                  className="px-3 py-1 border border-slate-700 text-slate-400 rounded-full"
                >
                  {tech}
                </span>
              ))}
